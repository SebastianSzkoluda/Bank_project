...
public class KierownikFrame {
	 ...
	 */
	public static void kierownikFrame() {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					KierownikFrame window = new KierownikFrame(login);
	 ...
	 */
	public KierownikFrame(String login) {
		this.login = login;
		initialize();
		getDataFromDatabase();
	}
...
private void initialize() {
		...
		frame = new JFrame("Okno kierownika");
	...
	private void getDataFromDatabase() {
		...
		String id = login;
    	String s1SQL = "SELECT * from kierownik where kierownik_id = " + id;
    	try {
    		ConnectionDBC con = new ConnectionDBC();
			PreparedStatement myStmt = con.getMyConn().prepareStatement(s1SQL);
			ResultSet rs = myStmt.executeQuery(s1SQL );
